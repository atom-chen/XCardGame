...
-- 将一个原始的标准json表格转化为二维数组型的table
function JsonParser.To2DirArr( fileName , indexPre)
	local content = string.fromfile(fileName)
	-- caosi_debug("xxxxx", content)
	local jsont = Json.Decode(content)
	...
	if indexPre then
		...
		for k, v in pairs(fields) do
			keyStr = indexPre.."_"..v
			kOrg = _G[keyStr]
			if kOrg and kOrg ~= k then
				Logger.error("JsonParser.To2DirArr table fields index not match ", kOrg, k)
			end
			_G[keyStr] = k
		end
	end
	...
	local ret = {}
	local t, value, err
	local parser = JsonParser.Value
	for k, v in pairs(values) do
		t = {}
		table.insert(ret , t)
		for i,vv in pairs(v)  do
			value, err = parser( types[i], vv)
			if not err then
				table.insert(t, value)
			else
				Logger.error("JsonParser.To2DirArr()  ", fileName, k, fields[i], err)
			end
		end
	end
	return ret
...
function JsonParser.ToStructList(fileName , needP2K)
    ...
    local jsont = CCFileUtils:sharedFileUtils():loadXml(fileName, true)
